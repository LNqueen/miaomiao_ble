    ...
    if ((gRFAL.state >= RFAL_STATE_MODE_SET) /*&& (gRFAL.TxRx.state == RFAL_TXRX_STATE_INIT )*/)
    {
        ...
        if (gRFAL.timings.FDTListen != RFAL_TIMING_NONE)
        {
            ...
            if (gRFAL.mode == RFAL_MODE_POLL_NFCA)
                FxTAdj += RFAL_FDT_LISTEN_A_ADJUSTMENT;
            else if (gRFAL.mode == RFAL_MODE_POLL_NFCA_T1T)
                FxTAdj += RFAL_FDT_LISTEN_A_ADJUSTMENT;
            else if (gRFAL.mode == RFAL_MODE_POLL_NFCB)
                FxTAdj += RFAL_FDT_LISTEN_B_ADJUSTMENT;
        ...
        if (rfalIsModePassiveComm(gRFAL.mode)) /* Passive Comms */
        {
            if ((gRFAL.TxRx.ctx.fwt != RFAL_FWT_NONE) && (gRFAL.TxRx.ctx.fwt != 0))
            {
                ...
                if (gRFAL.mode == RFAL_MODE_POLL_NFCA)
                    FxTAdj += RFAL_FWT_A_ADJUSTMENT;
                else if (gRFAL.mode == RFAL_MODE_POLL_NFCA_T1T)
                    FxTAdj += RFAL_FWT_A_ADJUSTMENT;
                else if (gRFAL.mode == RFAL_MODE_POLL_NFCB)
                    FxTAdj += RFAL_FWT_B_ADJUSTMENT;
        ...
        { /* Exchange receive buffer with internal buffer */
            ...
            gRFAL.TxRx.ctx.flags = RFAL_TXRX_FLAGS_CRC_TX_MANUAL | RFAL_TXRX_FLAGS_CRC_RX_KEEP | RFAL_TXRX_FLAGS_NFCIP1_OFF | (gRFAL.nfcvData.origCtx.flags & RFAL_TXRX_FLAGS_AGC_OFF) | RFAL_TXRX_FLAGS_PAR_RX_KEEP | RFAL_TXRX_FLAGS_PAR_TX_NONE;
    ...
    case RFAL_TXRX_STATE_RX_READ_DATA:
        ...
        if (((RFAL_MODE_POLL_NFCV == gRFAL.mode) || (RFAL_MODE_POLL_PICOPASS == gRFAL.mode)) && (gRFAL.TxRx.status == ERR_BUSY))
        {
            ...
            ret = iso15693VICCDecode(gRFAL.TxRx.ctx.rxBuf, gRFAL.fifo.bytesTotal,
                                     gRFAL.nfcvData.origCtx.rxBuf, rfalConvBitsToBytes(gRFAL.nfcvData.origCtx.rxBufLen), &offset, &len, gRFAL.nfcvData.ignoreBits, (RFAL_MODE_POLL_PICOPASS == gRFAL.mode));
            if (((ERR_NONE == ret) || (ERR_CRC == ret)) && (((uint32_t)RFAL_TXRX_FLAGS_CRC_RX_KEEP & gRFAL.nfcvData.origCtx.flags) == 0U) && ((len % RFAL_BITS_IN_BYTE) == 0U) && (len >= rfalConvBytesToBits(RFAL_CRC_LEN)))
            {

                len -= (uint16_t)rfalConvBytesToBits(RFAL_CRC_LEN); /* Remove CRC */
            }

            /* Restore original ctx */
            gRFAL.TxRx.ctx = gRFAL.nfcvData.origCtx;
            gRFAL.TxRx.status = ((ret != ERR_NONE) ? ret : ERR_BUSY);
            if (NULL != gRFAL.nfcvData.origCtx.rxRcvdLen)
            {
                *gRFAL.nfcvData.origCtx.rxRcvdLen = len;
            }
        }
#endif /* RFAL_FEATURE_NFCV */
